...
function lbl(extra = {}) {
  return { display: 'block', fontSize: 11, fontWeight: 700, color: '#4A4A4A', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 7, ...extra }
}
function card(extra = {}) {
  return { background: '#fff', border: '1px solid #E4E4E4', borderRadius: 14, padding: 20, ...extra }
}
      ...
      {sinhalaText(options.map(o => {
        ...
        return (
          <button key={o.value} onClick={() => onChange(o.value)} style={{
            ...
            background: active ? NAVY : '#ECECEC', color: active ? '#fff' : '#4A4A4A', transition: 'all 0.12s',
...
function StatPill({ label, value, loading }) {
  return (
    <div style={{ textAlign: 'center', padding: '10px 18px', background: '#F7F7F7', borderRadius: 12, border: '1px solid #E2E2E2' }}>
      <div style={{ fontSize: 18, fontWeight: 800, color: NAVY, lineHeight: 1 }}>{sinhalaText(loading ? '…' : value)}</div>
      <div style={{ fontSize: 10, color: '#8C8C8C', fontWeight: 600, marginTop: 3, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{sinhalaText(label)}</div>
    </div>
  )
}
...
function PostCard({ post, index, campaign, onSave, onDiscard, onRegenerate, saved, regenerating }) {
  ...
  const campColor = iUser ? '#8A6224' : ORANGE
  ...
  return (
    <div style={card({ marginBottom: 16 })}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: '#ECECEC', color: '#6E6E6E' }}>{sinhalaText(fmtLabel)}</span>
        <span style={{ fontSize: 11, color: '#8C8C8C', marginLeft: 'auto' }}>#{sinhalaText(index + 1)}</span>
      </div>

      {/* Hook */}
      {sinhalaText(post.hook && (
        <div style={{ fontSize: 12, fontWeight: 600, color: '#0B0B0B', background: '#F5EEE2', borderRadius: 8, padding: '8px 12px', marginBottom: 10, borderLeft: `3px solid ${NAVY}` }}>
          🪝 {sinhalaText(post.hook)}
        </div>
      ))}
      ...
      <textarea value={caption} onChange={e => setCaption(e.target.value)} rows={5}
        style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #E2E2E2', borderRadius: 10, fontSize: 13, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box', lineHeight: 1.6, marginBottom: 10 }} />

      {/* CTA */}
      {sinhalaText(post.cta && (
        <div style={{ fontSize: 12, color: '#4A4A4A', marginBottom: 8 }}>
      ...
      {sinhalaText(post.image_description && (
        <div style={{ fontSize: 12, color: '#6E6E6E', background: '#F7F7F7', borderRadius: 8, padding: '8px 12px', marginBottom: 10, lineHeight: 1.6 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {sinhalaText(post.slides.map((s, i) => (
              <div key={i} style={{ flex: '0 0 150px', border: '1px solid #E2E2E2', borderRadius: 10, padding: '10px 11px', fontSize: 11 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#8C8C8C', textTransform: 'uppercase', marginBottom: 4 }}>ස්ලයිඩය {sinhalaText(i + 1)}</div>
                <div style={{ fontWeight: 700, color: NAVY, marginBottom: 3, lineHeight: 1.3 }}>{sinhalaText(s.headline)}</div>
                <div style={{ color: '#6E6E6E', lineHeight: 1.4, marginBottom: 4 }}>{sinhalaText(s.body)}</div>
                {sinhalaText(s.design_note && <div style={{ fontSize: 10, color: '#8C8C8C', fontStyle: 'italic' }}>{sinhalaText(s.design_note)}</div>)}
              </div>
            )))}
          </div>
        </div>
      ...
      {sinhalaText(post.script && (
        <div style={{ fontSize: 12, color: '#4A4A4A', background: '#F7F7F7', borderRadius: 8, padding: '10px 12px', marginBottom: 10, whiteSpace: 'pre-wrap', lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <button onClick={handleCopy}
          style={{ padding: '7px 14px', background: '#ECECEC', color: '#4A4A4A', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
          {sinhalaText(copied ? '✓ Copied!' : '📋 Copy Row')}
        </button>
        <button onClick={() => onRegenerate(index)} disabled={regenerating}
          style={{ padding: '7px 14px', background: '#ECECEC', color: '#4A4A4A', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', opacity: regenerating ? 0.6 : 1 }}>
          {sinhalaText(regenerating ? '⏳' : '🔄')} නැවත සාදන්න
        </button>
        <button onClick={() => onDiscard(index)}
          style={{ padding: '7px 14px', background: '#FBEDEB', color: '#5E3F14', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
          🗑 ඉවත් කරන්න
        </button>
      </div>
    </div>
    ...
    <div style={{ fontFamily: 'Montserrat, system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        <button onClick={refreshStats} title="සංඛ්‍යා දත්ත නැවුම් කරන්න"
          style={{ padding: '6px 12px', background: '#ECECEC', border: 'none', borderRadius: 10, fontSize: 12, color: '#6E6E6E', cursor: 'pointer', alignSelf: 'center' }}>
          ↻ නැවුම් කරන්න
        </button>
      ...
      {sinhalaText(missingKey && (
        <div style={{ background: '#F2EADC', border: '1px solid #DCC9A4', borderRadius: 12, padding: '14px 18px', marginBottom: 20 }}>
          <div style={{ fontWeight: 700, color: '#242424', marginBottom: 6 }}>⚠️ Anthropic API යතුර සකසා නැත</div>
          <div style={{ fontSize: 12, color: '#0B0B0B', lineHeight: 1.6 }}>
      ...
      <div style={card({ marginBottom: 20 })}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#0B0B0B', marginBottom: 18 }}>⚙️ අන්තර්ගත සැකසුම්</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18, marginBottom: 20 }}>
          <div>
            ...
            <select value={angle} onChange={e => setAngle(e.target.value)}
              style={{ width: '100%', padding: '10px 13px', border: '1.5px solid #E2E2E2', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', background: '#fff', cursor: 'pointer' }}>
          ...
          <div>
            ...
            <input type="date" value={scheduleDate} onChange={e => setScheduleDate(e.target.value)}
              style={{ width: '100%', padding: '10px 13px', border: '1.5px solid #E2E2E2', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', boxSizing: 'border-box' }} />
            <div style={{ fontSize: 10, color: '#8C8C8C', marginTop: 4 }}>පළ කිරීම් අතර දින 2ක පරතරයක්</div>
          </div>
        </div>
        ...
        <button onClick={handleGenerate} disabled={generating || missingKey}
          style={{
            padding: '12px 28px', background: generating ? '#8C8C8C' : ORANGE, color: '#fff', border: 'none',
      ...
      {/* Error */}
      {sinhalaText(error && (
        <div style={{ background: '#FBEDEB', border: '1px solid #F2C9C3', borderRadius: 10, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: '#5E3F14' }}>
          ⚠️ {sinhalaText(error)}
        </div>
      ))}
      ...
      {sinhalaText(posts.length > 0 && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#0B0B0B' }}>සෑදූ පළ කිරීම් ({sinhalaText(posts.length)})</div>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <button onClick={handleCopyAll}
                style={{ padding: '8px 16px', background: GOLD, color: '#0B0B0B', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                {sinhalaText(copyAllMsg || '📋 Copy All Rows')}
              </button>
              <a href={SHEET_URL} target="_blank" rel="noreferrer"
                style={{ padding: '8px 16px', background: '#ECECEC', color: '#4A4A4A', borderRadius: 8, fontSize: 12, fontWeight: 700, textDecoration: 'none', display: 'inline-block' }}>
      ...
      {sinhalaText(generating && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {sinhalaText(Array.from({ length: numPosts }).map((_, i) => (
            <div key={i} style={{ ...card(), height: 200, background: 'linear-gradient(90deg,#ECECEC 25%,#E2E2E2 50%,#ECECEC 75%)', animation: 'pulse 1.5s ease infinite', backgroundSize: '200% 100%' }} />
          )))}
        </div>
      ))}
    </div>
