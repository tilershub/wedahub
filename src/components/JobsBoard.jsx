...
export default function JobsBoard({initialQuery=''}) {
 ...
 return <section><div className="wh-filters"><label>වැඩය හෝ සේවාව<input value={query} onChange={e=>{setQuery(e.target.value);setPage(0)}} placeholder="උදා: පිරිසිදු කිරීම, නිර්මාණ, අලුත්වැඩියා…"/></label><label>ප්‍රදේශය<select value={district} onChange={e=>{setDistrict(e.target.value);setPage(0)}}><option value="">ශ්‍රී ලංකාව පුරා</option>{DISTRICTS_EN.map(d=><option key={d} value={d}>{si(d)}</option>)}</select></label></div>
 {loading?<p role="status">වැඩ සොයමින්…</p>:error?<p role="alert">{error}</p>:rows.length?<div className="wh-provider-grid">{rows.slice(0,20).map(job=><JobCard key={job.id} job={job}/>)}</div>:<div className="wh-empty"><h2>දැනට ගැළපෙන වැඩ නැහැ.</h2><p>වෙනත් ප්‍රදේශයක් හෝ සෙවුම් වචනයක් උත්සාහ කරන්න. අලුත් අවස්ථා සඳහා පසුව නැවත බලන්න.</p></div>}
